...
;(function() {
  function errMsg(errCode, msg) {
    return (
      (msg || "") + " (SystemJS Error#" + errCode + " " + "https://git.io/JvFET#" + errCode + ")"
    )
  }
  ...
  function targetWarning(code, match, target, msg) {
    console.warn(
      errMsg(code, "Package target " + msg + ", resolving target '" + target + "' for " + match)
    )
  }
  ...
  systemJSPrototype.createScript = function(url) {
    ...
    if (!url.startsWith(baseOrigin + "/")) script.crossOrigin = "anonymous"
  ...
  systemJSPrototype.instantiate = function(url, firstParentUrl) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      script.addEventListener("error", function() {
        reject(
          Error(
            errMsg(3, "Error loading " + url + (firstParentUrl ? " from " + firstParentUrl : ""))
          )
        )
      })
        ...
        return this.fetch(url).then(function(res) {
          ...
          if (!contentType)
            throw Error(
              errMsg(
                4,
                'Missing header "Content-Type", loading ' + url + (parent ? " from " + parent : "")
              )
            )
